...
namespace circle.Entities.formas
{
    public class Hexagono : Forma
    {
        ...
        {
            int tamanho = 100;
            var pontos = new Point[6];
            var cx = paintBox.Width / 2;
            var cy = paintBox.Height / 2;
            double angulo = 2 * Math.PI / 6; //Angulo 120 graus cada vertice
            ...
            using (var gradiente = new LinearGradientBrush(new Rectangle(cx - tamanho, cy - tamanho, tamanho * 2, tamanho * 2), Color.Yellow, Color.Black, LinearGradientMode.Horizontal))
            {
