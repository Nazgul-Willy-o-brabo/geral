...
namespace circle.Entities.formas
{
    public class Octagono : Forma
    {
        ...
        {
            int tamanho = 100;
            var pontos = new Point[8];
            var cx = paintBox.Width / 2;
            var cy = paintBox.Height / 2;
            double angulo = 2 * Math.PI / 8; //Angulo 120 graus cada vertice
            ...
            using (var gradiente = new LinearGradientBrush(new Rectangle(cx - tamanho, cy - tamanho, tamanho * 2, tamanho * 2), Color.Orange, Color.Purple, LinearGradientMode.Horizontal))
            {
